(* let print_array arr = print_string "["; Array.map (fun a -> print_int a;
...
let rec interp bytecode pc stack sp =
  ...
  then (
    ...
    stack.(sp - 1) <- v1 - v2;
    interp bytecode (pc + 1) stack (sp - 1)
    (* else if instr = 3 then (* LT *) let v2 = stack.(sp) in let v1 = stack.(sp
...
(* RET *)
(* let code2 = Array.make 20 (-1) in let stack2 = Array.make 20 (-1) in
...
(* return 9 *) print_newline ()

(* LT true *)
(* let code4 = Array.make 100 (-1) in let stack4 = Array.make 20 (-1) in
   stack4.(0) <- 2; stack4.(1) <- 3; code4.(0) <- 3; code4.(1) <- 30; print_int
   (interp code4 0 stack4 1) (* return 1*); print_newline ();

   (* LT false *) let code5 = Array.make 100 (-1) in let stack5 = Array.make 20
