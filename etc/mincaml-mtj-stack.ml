...
let extend_env env var value name = if var = name then value else env name
let make_env vars vals = List.fold_left2 extend_env null_env vars vals

(* the core interpreter *)
let rec interp fundefs exp env =
  match exp with
  | Int n -> n
  | Var v -> env v
  ...
  | LT (e1, e2) ->
    if interp fundefs e1 env < interp fundefs e2 env then 1 else 0
  ...
  | Let (var, exp, body) ->
    interp fundefs body (extend_env env var (interp fundefs exp env))
;;
...
module VM : sig
  ...
  type inst =
    ...
    (* v1::...::vn::s -> vn::v1::...::vn::s *)
    | HALT (* n -> . *)
    (* The next instruction assumes that the stack has (1) o values as the
       parameter to this frame, (2) the return address from this frame, (3) l
       values as the local variables in this frame, and (4) n values as the new
       parameter, and deletes (1) and (3), moves (2) to the top of the stack.

       before: (stack top) [n new args][l local vars][ret][o old
       args]...(bottom) after: (stack top) [ret][n new args]...(bottom) *)
    | FRAME_RESET (* o l n *)
    | POP1 (* n2::n1::s -> n2::s *)
    | JUMP (* addr *)
    | METHOD_ENTRY
    (* the following constructors do not represent instructions but are defined
       for expressing operands of some instructions as well as label
  ...
  (* operand stack
     ...
     the arry is not changed over the push/pop operations, those operations
     return a new pair just for convenience. If we switch to the MinCaml
     implementation, we should decouple the pair. *)
 ...
 ;;
  ...
  let take : stack -> int -> int = fun (sp, stack) n -> stack.(sp - n - 1)
 ...
 ;;
  ...
  let dump_stack (sp, stack) =
    let rec loop i =
      if i = sp then "" else string_of_int stack.(i) ^ ";" ^ loop (i + 1)
    in
    "[" ^ loop 0 ^ "]"
  ;;
  ...
  let rec interp code pc stack =
    ...
    then fst (pop stack)
      ...
      | JUMP_IF_ZERO (* addr *) ->
        ...
        if v = 0 then interp code addr stack else interp code pc stack
      | CALL (* addr *) ->
        (* calling a function will create a new operand stack and lvars *)
        let addr, pc = fetch code pc in
        let stack = push stack pc in
        (* save return address *)
        (* (let (sp,s)=stack in
         *  if 2<sp then
 ...
 ;;
  ...
  let test_ex arity pc name instrs ostack lvars expected =
    ...
    let ostack =
      Array.init max_stack_depth (fun i ->
    ...
    then Printf.printf "OK: %s\n" name
      ...
      failwith (Printf.sprintf "NG: %s expected=%d actual=%d\n" name expected v)
  ;;
  ...
  let _ =
    test
      "FRAME_RESET" (* TOP 6 5 4 3 2 1 BOTTOM *)
      [ FRAME_RESET
      ; Literal 2
      ; Literal 1
      ; Literal 1
  ...
  let _ = test "POP1" [ POP1; ADD; HALT ] [ 1; 2; 3 ] [] 4
  let _ = test_ex 0 2 "CALL simple" [ HALT; CALL; Literal 1 ] [] [] 4

  let _ =
    test_ex 0 1 "RET simple" [ RET; Literal 1; ADD; HALT ] [ 4; 5; 3; 6 ] [] 10
  ;;
...
module Compiler : sig
  ...
  let lookup env var =
    fst
      (List.find (fun (_, v) -> var = v) (List.mapi (fun idx v -> idx, v) env))
  ;;
  ...
  (* computes the number of arguments to this frame. The stack has a shape like
     [...local vars...][ret addr][..args...], the return address position from
     the top indicates the number of local variables on top of the return
     address. *)
  ...
  let rec compile_exp fenv exp env =
    VM.(
      ...
      | Add (e1, e2) ->
        compile_exp fenv e1 env @ compile_exp fenv e2 (shift_env env) @ [ ADD ]
      | Mul (e1, e2) ->
        compile_exp fenv e1 env @ compile_exp fenv e2 (shift_env env) @ [ MUL ]
      | LT (e1, e2) ->
        compile_exp fenv e1 env @ compile_exp fenv e2 (shift_env env) @ [ LT ]
      | If (cond, then_exp, else_exp) ->
             ...
             (fst
                (List.fold_left
                   (fun (rev_code_list, env) exp ->
                     compile_exp fenv exp env :: rev_code_list, shift_env env)
                   ([], env)
                   rands)))
        @ [ CALL; Lref fname ]
      (* call using a label *)
      (* self tail calls are compiled with FRAME_RESET which moves the computed
         arguments to the position of the actual parameters in the current
         frame. *)
      | TCall (fname, rands) ->
  ...
  let rec tail_elim fname = function
    ...
    | Call (fname', rands) ->
      if fname = fname' then TCall (fname', rands) else Call (fname', rands)
    | Let (var, exp, body) -> Let (var, exp, tail_elim fname body)
    | others -> others
  ;;
  ...
  let resolve_labels instrs =
    List.filter
      (function VM.Ldef _ -> false | _ -> true)
  ...
  let compile_funs fundefs =
    let fenv name =
      fst
        (List.find
           (fun (_, { name = n }) -> name = n)
           (List.mapi (fun idx fdef -> idx, fdef) fundefs))
    in
    Array.of_list
  ...
  let print_code_pair code1 code2 =
    Printf.sprintf
      "code1: %s\ncode2: %s\n"
      (string_of_code_list code1)
      (string_of_code_list code2)
  ;;

  let test_ex compiler name exp expected =
    let actual =
      compiler (fun _ -> 47) exp (extend_env (build_arg_env [ "x"; "y" ]) "w")
    in
    if actual = expected
    then Printf.printf "%s: %s\n" name "OK"
    else
      failwith
        (Printf.sprintf "%s: NG\n%s" name (print_code_pair expected actual))
  ;;
  ...
  let _ =
    test "Add" (Add (Var "x", Var "x")) [ DUP; Literal 3; DUP; Literal 4; ADD ]
  ;;
  ...
  let _ =
    test
      "Call"
      (Call ("f", [ Var "x"; Var "x" ]))
      [ DUP; Literal 3; DUP; Literal 4; CALL; Lref "f" ]
  ;;

  let _ =
    test
      "Let"
      (Let ("z", Int 123, Add (Var "z", Var "y")))
      [ CONST; Literal 123; DUP; Literal 0; DUP; Literal 4; ADD; POP1 ]
  ;;

  let _ =
    test
      "TCall"
      (TCall ("f", [ Int 123; Int 456; Int 789 ]))
      [ CONST
      ; Literal 123
      ; CONST
      ; Literal 456
  ...
  let _ =
    test_ex
      (fun fenv exp env -> resolve_labels (compile_exp fenv exp env))
      "resolve"
      (If (Int 123, Int 456, Int 789))
      [ CONST
      ; Literal 123
...
let rec sum_ocaml n = if n = 0 then 0 else n + sum_ocaml (n - 1)
...
let eq =
  ...
  ; args = [ "x"; "y" ]
  ; body =
      If (LT (Var "x", Var "y"), Int 0, If (LT (Var "y", Var "x"), Int 0, Int 1))
  }
;;
...
(* compliation of gcd yields the following bytecode (local variable indices,
   function indices are replaced with variable names like a, b, and sub for
   readability. But the key points are, the tail calls are eliminated into
   jump-to-the-beginning instructions (i.e., CONST 0; JUMP_IF_ZERO 1).

   [|Literal 2; 1:LOAD a; LOAD b; CALL eq; JUMP_IF_ZERO 15; LOAD a; CONST 0;
   JUMP_IF_ZERO 58; 15:LOAD a; LOAD b; LT; JUMP_IF_ZERO 42; LOAD a; LOAD b; LOAD
   a; CALL sub; STORE b; STORE a; CONST 0; JUMP_IF_ZERO 1; CONST 0; JUMP_IF_ZERO
   58; 42:LOAD a; LOAD b; CALL sub; LOAD b; STORE b; STORE a; CONST 0;
   JUMP_IF_ZERO 1; 58:RET|] *)

(* let _ = assert (21 = ccexe [eq;sub;gcd] "gcd" [252;105]) *)

let rec mod_ n m = if n < m then n else mod_ (n - m) m

let rec prime_test cand i =
  let i2 = i * i in
  if cand < i2 then 1 else if mod_ cand i = 0 then 0 else prime_test cand (i + 1)
;;

let is_prime cand = if 2 < cand then prime_test cand 2 else 1
...
let mod_ =
  ...
  ; body =
      If
        ( LT (Var "n", Var "m")
        , Var "n"
        , Call ("mod", [ Call ("sub", [ Var "n"; Var "m" ]); Var "m" ]) )
  }
;;
  ...
  ; body =
      Let
        ...
        , If
            ...
            , Int 1
            , If
                ( Call ("eq", [ Call ("mod", [ Var "cand"; Var "i" ]); Int 0 ])
                , Int 0
                , Call ("prime_test", [ Var "cand"; Add (Var "i", Int 1) ]) ) )
        )
  }
;;
...
(* let _ = assert (1 = ccexe [sub;eq;mod_;prime_test;is_prime] "prime_test"
   [3;2]) *)

let _ =
  assert (1 = ccexe [ sub; eq; mod_; prime_test; is_prime ] "is_prime" [ 7 ])
;;

let _ =
  assert (0 = ccexe [ sub; eq; mod_; prime_test; is_prime ] "is_prime" [ 9 ])
;;

let _ =
  assert (1 = ccexe [ sub; eq; mod_; prime_test; is_prime ] "is_prime" [ 83 ])
;;
