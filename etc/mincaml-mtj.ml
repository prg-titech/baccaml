...
module VM : sig
  ...
  let int_of_inst = function Literal n -> n | inst -> index_of inst insts
 ...
 ;;
  ...
  let rec interp fundefs code pc stack lvars =
    ...
    | CALL (* function_id *) ->
      ...
      interp fundefs code pc stack lvars
    | RET ->
      (* when returning from a function the operand stack should contain one and
         only one value *)
      if sp = 1 then fst (pop stack) else raise (Failure "not one stack at RET")
  ;;
  ...
  let run_bin : fundef_bin_t -> int =
   fun fundefs ->
    ...
    if arity = 0
    then interp fundefs target_code 1 stack lvars
    else raise (Failure "function 0 should be nullary.")
 ;;
  ...
  let run_asm : fundef_asm_t -> int =
   fun fundefs -> run_bin (Array.map (Array.map int_of_inst) fundefs)
 ;;
  ...
  let test_ex arity pc name instrs ostack lvars expected =
    ...
    let ostack =
      Array.init 64 (fun i -> if i < sp then List.nth ostack i else 0)
    in
    let lvars =
      Array.init 256 (fun i ->
          if i < List.length lvars then List.nth lvars i else 0)
    in
  ...
  let _ = test "STORE" [ STORE; Literal 0; LOAD; Literal 0; RET ] [ 3 ] [] 3
  ...
  let _ =
    test_ex
      ...
      4
      "CALL simple"
      [ LOAD; Literal 0; (* 1;2 *) RET; (* 3 *) CALL; Literal 0; (* 4;5 *) RET ]
      (* 6 *)
      [ 9; 8 ]
      []
      9
...
module Compiler : sig
    ...
    let (Some idx) =
      List.fold_left
        (function
          ...
          | None -> fun v -> if v = var then Some 0 else None)
        None
        env
    in
    idx
  ...
  let rec compile_exp fenv exp env =
    VM.(
      ...
      | Add (e1, e2) ->
        compile_exp fenv e1 env @ compile_exp fenv e2 env @ [ ADD ]
      | Mul (e1, e2) ->
        compile_exp fenv e1 env @ compile_exp fenv e2 env @ [ MUL ]
      | LT (e1, e2) ->
        compile_exp fenv e1 env @ compile_exp fenv e2 env @ [ LT ]
      | If (cond, then_exp, else_exp) ->
        ...
        @ [ Ldef l2 ]
      | Call (fname, rands) ->
        List.flatten (List.map (fun exp -> compile_exp fenv exp env) rands)
        @ [ CALL; Literal (fenv fname) ]
      (* self tail calls are compiled into a series of store followed by a jump
         to zero. This compilation assumes that only self-tail calls are using
         TCall node. *)
      | TCall (fname, rands) ->
  ...
  let compile_fun_body fenv arity exp env =
    Array.of_list
      ((VM.Literal arity :: resolve_labels (compile_exp fenv exp env))
      @ [ VM.RET ])
  ;;
  ...
  let _ =
    test
      "Call"
      (Call ("f", [ Int 123; Int 456 ]))
      [ CONST; Literal 123; CONST; Literal 456; CALL; Literal 47 ]
  ;;
  ...
  let _ =
    test
      "Let"
      (Let ("z", Int 123, Var "z"))
      [ CONST; Literal 123; STORE; Literal 2; LOAD; Literal 2 ]
  ;;

  let _ =
    test
      "TCall"
      (TCall ("f", [ Int 123; Int 456 ]))
      [ CONST
      ; Literal 123
      ; CONST
      ; Literal 456
