...
let rec init_array a i =
  if i < 0 then a else (a.(i) <- i; init_array a (i-1)) in
...
print_int(a.(0)); print_int(a.(1)); print_int(a.(2)); print_newline();

(* cast_fAII returns a's address as an integer *)
print_int(cast_fAII(a)); print_newline();

(* b should be an array from the second element of a *)
let b = cast_fIAI(cast_fAII(a)+4) in
print_int(b.(0)); print_int(b.(1)); print_int(b.(2)); print_newline();

(* a and b are sharing memory *)
a.(1) <- 99;
print_int(b.(0)); print_int(b.(1)); print_int(b.(2)); print_newline();

a.(2) <- cast_fAII(b); print_int (a.(2))
