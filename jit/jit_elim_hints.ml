...
let hints = [ "min_caml_jit_merge_point"; "min_caml_can_enter_jit" ]
let contain_hints (Id.L x) = List.mem (String.get_name x) hints

let rec elim_hints = function
  | Ans e -> Ans (elim_hints' e)
  | Let (x, CallDir (id_l, args, fargs), t) when contain_hints id_l ->
    elim_hints t
...
let elim_hints_and_rename typ fundefs =
  ...
  | Some interp ->
    (elim_hints_fundef interp |> rename_interp |> Jit_annot.annotate_fundef typ)
    :: fundefs
  | None -> fundefs
;;
...
let%test "test_jit_elim_hint" =
  let testee =
    ...
    Let
      ( ("x.1", Int)
      , CallDir (Id.L "min_caml_jit_merge_point.1", [], [])
      , Let
          ( ("y.2", Int)
          , Set 47
          , Let (("z.3", Int), Add ("y.2", C 93), Ans (Mov "z.3")) ) )
  and expected =
    Let
      ( ("y.2", Int)
      , Set 47
