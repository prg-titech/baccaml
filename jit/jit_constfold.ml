...
and replace_exp trg = function
  ...
  | IfEq (x, y, t1, t2) when x /=/ trg ->
    IfEq (trg, y, replace trg t1, replace trg t2)
  | IfLE (x, y, t1, t2) when x /=/ trg ->
    IfLE (trg, y, replace trg t1, replace trg t2)
  | IfGE (x, y, t1, t2) when x /=/ trg ->
    IfGE (trg, y, replace trg t1, replace trg t2)
  ...
  | SIfEq (x, y, t1, t2) when x /=/ trg ->
    SIfEq (trg, y, replace trg t1, replace trg t2)
  | SIfLE (x, y, t1, t2) when x /=/ trg ->
    SIfLE (trg, y, replace trg t1, replace trg t2)
  | SIfGE (x, y, t1, t2) when x /=/ trg ->
    SIfGE (trg, y, replace trg t1, replace trg t2)
  ...
  | CallCls (x, ys, zs) ->
    CallCls
      ( x
      , List.map (fun y -> if y /=/ trg then trg else y) ys
      , List.map (fun z -> if z /=/ trg then trg else z) zs )
  | CallDir (x, ys, zs) ->
    CallDir
      ( x
      , List.map (fun y -> if y /=/ trg then trg else y) ys
...
let rec iter ?(n = 2) t =
  let r = folding t in
  if t = r then r else if n = 0 then r else iter ~n:(n - 1) r
;;

let rec iter_fundef ?(n = 2) fundef =
  let { name; args; fargs; body; ret } = fundef in
  let body = iter ~n body in
