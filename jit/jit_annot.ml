...
let rec annotate_t typ t =
  ...
  | Let (x, CallDir (id_l, args, fargs), t) when id_l = Id.L "min_caml_is_mj" ->
    ...
    | Ans (IfEq (_, _, t1, t2)) ->
      ...
      (match typ with `Meta_method -> t2 | `Meta_tracing -> t1)
    | _ -> Let (x, CallDir (id_l, args, fargs), annotate_t typ t))
...
let annotate_fundef typ { name; args; fargs; body; ret } =
  let rec loop typ n body =
    ...
    then body
      ...
      if body = t then body else loop typ (n - 1) t)
  in
...
let rec annotate typ (Prog (table, const, fundefs, main) as p) =
  ...
  let new_fundefs =
    { name; args; fargs; ret; body = loop typ 100 body } :: other_fundefs
  in
