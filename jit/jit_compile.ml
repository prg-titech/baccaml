...
let create_static_dir _ =
  ...
  try
    if Sys.is_directory static_dir then (rmdir (); mkdir ());
  with Sys_error _ ->
    mkdir ()
...
let create_archive obj_name other_objs =
  ...
  create_static_dir ();
  sp "ar rcs _static/%s %s %s" archive_name obj_name (
    match other_objs with
    | Some other_objs -> String.concat " " other_objs
    | None -> "")
  |> Unix.system
  |> function | Unix.WEXITED (i) when i = 0 -> Ok archive_name
...
let compile_dyn trace_name =
  ...
  |> Unix.system
  |> (function Unix.WEXITED i when i = 0 -> () | _ -> failwith "compilation failed.");
  sp "gcc -m32 -g -o %s -shared -fPIC %s -L./_static" so asm_name
  |> Unix.system
  |> function
    Unix.WEXITED (i) when i = 0 ->
    begin match create_archive obj_name None with
      | Ok _ -> Ok trace_name
      | e -> e
    end
...
let compile_dyn_with_so tname others =
  ...
  let other_archives = others |> List.map (fun so -> "-l" ^ so) |> String.concat " " in
  sp "gcc -m32 -g -o %s %s -shared -fPIC -DRUNTIME -ldl -L./_static %s"
    so asm other_archives
  |> Unix.system
  |> function
