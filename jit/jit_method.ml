...
let rec mj p reg mem ({trace_name; red_names; index_pc; merge_pc; function_pcs; bytecode} as env) fenv =
  function
...
and mj_exp p reg mem ({index_pc; merge_pc; bytecode} as env) fenv =
  function
...
let run prog reg mem ({trace_name; red_names; index_pc; merge_pc; bytecode} : env) =
  ...
  let fenv name = Fundef.find_fuzzy prog ~name:name in
