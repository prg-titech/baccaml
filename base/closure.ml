...
type t =
  ...
  | SIfLE of Id.t * Id.t * t * t
  | SIfEq of Id.t * Id.t * t * t
...
let rec g env known = function
  ...
  | KNormal.SIfEq (x, y, e1, e2) -> SIfEq (x, y, g env known e1, g env known e2)
  | KNormal.SIfLE (x, y, e1, e2) -> SIfLE (x, y, g env known e1, g env known e2)
  ...
  | KNormal.LetRec
    ...
    (* 関数定義let rec x y1 ... yn = e1 in e2の場合は、
       xに自由変数がない(closureを介さずdirectに呼び出せる) と仮定し、knownに追加してe1をクロージャ変換してみる *)
    let toplevel_backup = !toplevel in
    let env' = M.add x t env in
    let known' = S.add x known in
    let e1' = g (M.add_list yts env') known' e1 in
    (* 本当に自由変数がなかったか、変換結果e1'を確認する *)
    (* 注意: e1'にx自身が変数として出現する場合はclosureが必要! (thanks to nuevo-namasute and
       azounoman; test/cls-bug2.ml参照) *)
    ...
    then
      ...
      (* 出現していたら削除しない *)
    else (* Format.eprintf "eliminating closure(s) %s@." x; *) e2'
  (* 出現しなければMakeClsを削除 *)
