...
let rec extern_return_type t =
  match t with Type.Unit -> "void" | _ -> string_of_type t
;;
  ...
  M.iter
    (fun x t ->
      ...
      | Type.Array _ | Type.Tuple _ ->
        r := !r ^ Printf.sprintf "extern Value %s[];\n" x
...
let rec make_typedefs (f : fundef list) (g : string list) (h : string list) =
  ...
  | _ :: _ ->
    ...
    let g', h' =
      if not (List.exists (fun n -> n = name) g)
      then name :: g, typedef :: h
      else g, h
    in
...
(*This function will translate a single line of the OCaml intermediate code and
  decide the translated C version*)
let rec trans_exp
    r'
    (rt : Type.t)
    (t_env : (string * Type.t) list)
    (typedef_names : string list)
    (env_name : string)
    (func_name : string)
  =
  let r = alpha_convert r' in
  ...
  | Let ((x, t), e1, e2) ->
    ...
    let t2 =
      trans_exp r rt ((x, t) :: t_env) typedef_names env_name func_name e2
    in
  ...
  | AppCls (x, ys) ->
    let x' = alpha_convert x in
    if x' = func_name
    then Printf.sprintf "  %s = %s_fun(%s, env, %s);" r x' (list_params ys) x'
    else (
      let types =
        "fun_"
        ^ typedef_of_type rt
  ...
  | AppDir (Id.L l, xs) ->
    ...
    | "min_caml_create_float_array" | "min_caml_create_array" ->
      ...
      let cls =
        if string_of_type init_typ = "Closure*"
        then make_closure init init "env"
        else ""
      in
      Printf.sprintf
        "%s%s = make_%s_array(%s, %s);"
        cls
        r
        (type_for_array init_typ)
        (alpha_convert size)
        (alpha_convert init_n)
...
(*Compiles Min-Caml code through closure conversion, then translates the
  resulting intermediate code into C*)
let translate s =
  let funcs, mainf =
