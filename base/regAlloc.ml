...
let rec g dest cont regenv = function
  ...
  | Let (((x, t) as xt), exp, e) ->
    (try assert (not (M.mem x regenv)) with
    | e ->
      Printf.eprintf "%s cannot be allocated.\n" x;
      raise e);
...
and g' dest cont regenv = function
  ...
  | IfFLE (x, y, e1, e2) as exp ->
    g'_if
      ...
      e1
      e2
  | SIfEq (x, y', e1, e2) as exp ->
    g'_if
      dest
      cont
      regenv
      exp
      (fun e1' e2' -> SIfEq (find x Type.Int regenv, find' y' regenv, e1', e2'))
      e1
      e2
  | SIfLE (x, y', e1, e2) as exp ->
    g'_if
      dest
      cont
      regenv
      exp
      (fun e1' e2' -> SIfLE (find x Type.Int regenv, find' y' regenv, e1', e2'))
      e1
      e2
  | SIfGE (x, y', e1, e2) as exp ->
    g'_if
      dest
      cont
      regenv
      exp
      (fun e1' e2' -> SIfGE (find x Type.Int regenv, find' y' regenv, e1', e2'))
      e1
      e2
  | SIfFEq (x, y, e1, e2) as exp ->
    g'_if
      dest
      cont
      regenv
      exp
      (fun e1' e2' ->
        SIfFEq (find x Type.Float regenv, find y Type.Float regenv, e1', e2'))
      e1
      e2
  | SIfFLE (x, y, e1, e2) as exp ->
    g'_if
      dest
      cont
      regenv
      exp
      (fun e1' e2' ->
        SIfFLE (find x Type.Float regenv, find y Type.Float regenv, e1', e2'))
      e1
      e2
