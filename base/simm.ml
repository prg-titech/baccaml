...
and g' env = function (* 各命令の即値最適化 (caml2html: simm13_gprime) *)
  ...
  | SIfEq(x, V(y), e1, e2) when M.mem y env -> SIfEq(x, C(M.find y env), g env e1, g env e2)
  | SIfLE(x, V(y), e1, e2) when M.mem y env -> SIfLE(x, C(M.find y env), g env e1, g env e2)
  | SIfGE(x, V(y), e1, e2) when M.mem y env -> SIfGE(x, C(M.find y env), g env e1, g env e2)
  | SIfEq(x, V(y), e1, e2) when M.mem x env -> SIfEq(y, C(M.find x env), g env e1, g env e2)
  | SIfLE(x, V(y), e1, e2) when M.mem x env -> SIfLE(y, C(M.find x env), g env e1, g env e2)
  | SIfGE(x, V(y), e1, e2) when M.mem x env -> SIfGE(y, C(M.find x env), g env e1, g env e2)
  | IfEq(x, y', e1, e2) -> IfEq(x, y', g env e1, g env e2)
  | IfLE(x, y', e1, e2) -> IfLE(x, y', g env e1, g env e2)
  | IfGE(x, y', e1, e2) -> IfGE(x, y', g env e1, g env e2)
  | IfFEq(x, y, e1, e2) -> IfFEq(x, y, g env e1, g env e2)
  | IfFLE(x, y, e1, e2) -> IfFLE(x, y, g env e1, g env e2)
  | SIfEq(x, y', e1, e2) -> SIfEq(x, y', g env e1, g env e2)
  | SIfLE(x, y', e1, e2) -> SIfLE(x, y', g env e1, g env e2)
  | SIfGE(x, y', e1, e2) -> SIfGE(x, y', g env e1, g env e2)
  | SIfFEq(x, y, e1, e2) -> SIfFEq(x, y, g env e1, g env e2)
  | SIfFLE(x, y, e1, e2) -> SIfFLE(x, y, g env e1, g env e2)
  | e -> e
