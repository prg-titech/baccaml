...
and print_exp = function
  ...
  | Sub (x, y) -> print_string "Sub ("; print_string x; print_semi_colon ();
                  print_id_or_imm y; print_string ")"
  | Mul (x, y) -> print_string "Mul ("; print_string x; print_semi_colon ();
                  print_id_or_imm y; print_string ")"
...
let rec fv_exp = function
  ...
  | SIfEq(x, y', e1, e2) | SIfLE(x, y', e1, e2) | SIfGE(x, y', e1, e2)-> x :: fv_id_or_imm y' @ remove_and_uniq S.empty (fv e1 @ fv e2) (* uniq here just for efficiency *)
  | IfFEq(x, y, e1, e2) | IfFLE(x, y, e1, e2)
  | SIfFEq(x, y, e1, e2) | SIfFLE(x, y, e1, e2) ->
