...
let rec interp regs a bytecode pc =
  ...
  then (
    ...
    if a > -2
    then interp regs a bytecode target
    else interp regs a bytecode (pc + 2))
  else if instr = 1
  then (
  ...
  then interp regs (a - 1) bytecode (pc + 1)
  else if instr = 5
  then a
  else if instr = 6
  then (
  ...
  then interp regs a bytecode (pc + 1)
  else -1
in
...
let rec loop n =
  ...
  then ()
    ...
    let _ = interp regs 500000 bytecode 0 in
    loop (n - 1))
in
