...
let make_header () = Printf.sprintf "#include\"csyntax.c\"\n\n"
...
let rec typedef_for_funcs t =
  match t with Type.Fun (l, r) -> typedef_of_type r | _ -> assert false

(*Used for the return type of a function*)
let rec get_return_type t =
  match t with Type.Fun (l, r) -> string_of_type r | _ -> assert false

let rec extern_return_type t = match t with Type.Unit -> "void" | _ -> string_of_type t
...
let set_environment fv =
  ...
  | _ :: _ ->
      List.mapi
        (fun i l ->
          let n, t = l in
          Printf.sprintf "%s %s = env[%d].%s;\n" (string_of_type t) (alpha_convert n) i
            (type_for_union t) )
        fv
...
let make_closure r f env =
  ...
  Printf.sprintf
    "Closure* %s = closure_malloc();\n%s -> fp = (Function)%s_fun;\n%s -> env = %s;\n" r'
    r' (alpha_convert f) r' env
...
let list_params l =
  List.fold_left
    (fun acc s ->
      ...
      match acc with "" -> s' | _ -> acc ^ ", " ^ s' )
    "" l

(*Return the result of the function.*)
let end_function r = Printf.sprintf "\nreturn %s;\n}" (alpha_convert r)
...
let rec make_typedefs (f : fundef list) (g : string list) (h : string list) =
  ...
  | _ :: _ ->
      ...
      let a = elem.args in
      let signature =
        Printf.sprintf "(%s, Value *env, Closure *%s)"
          (List.fold_left
             (fun acc (s, typ) ->
               match acc with
               | "" -> acc ^ string_of_type typ ^ " " ^ alpha_convert s
               | _ -> acc ^ ", " ^ string_of_type typ ^ " " ^ alpha_convert s )
             "" a)
          (alpha_convert l)
      in
      let name =
        "fun_" ^ typedef_for_funcs t ^ "_"
        ^ List.fold_left
            (fun acc (s, typ) ->
              match acc with
              | "" -> typedef_of_type typ
              | _ -> acc ^ "_" ^ typedef_of_type typ )
            "" a
        ^ "_Value_Closure"
      in
      let typedef = get_return_type t ^ " " ^ name ^ signature in
      let g', h' =
        if not (List.exists (fun n -> n = name) g) then (name :: g, typedef :: h)
        else (g, h)
      in
...
let create_tuple xts y =
  List.mapi
    (fun i l ->
      let n, t = l in
      Printf.sprintf "%s %s = %s[%d].%s;\n" (string_of_type t) (alpha_convert n)
        (alpha_convert y) i (type_for_union t) )
    xts
  |> List.fold_left (fun acc s -> acc ^ "" ^ s) ""
  ...
  | _ :: _ ->
      List.mapi
        (fun i l ->
          ...
          Printf.sprintf "%s[%d].%s = %s;\n" (alpha_convert r) i (type_for_union typ)
            (alpha_convert l) )
        xs
      |> List.fold_left (fun acc s -> acc ^ "" ^ s) ""

(*This function will translate a single line of the OCaml intermediate code and decide
  the translated C version*)
let rec trans_exp r' (rt : Type.t) (t_env : (string * Type.t) list)
    (typedef_names : string list) (env_name : string) (func_name : string) =
  let r = alpha_convert r' in
  function
  ...
  | MakeCls ((x, t), {entry= Id.L l; actual_fv= ys}, e) -> (
    try
      ...
      let environment_creation =
        Printf.sprintf "Value *%s_env = safe_malloc(%d);\n%s" x' (List.length ys)
          ( List.mapi
              (fun i n ->
                let n' = alpha_convert n in
                let name, typ = List.find (fun (a, b) -> a = n) t_env in
                Printf.sprintf "%s_env[%d].%s = %s;\n" x' i (type_for_union typ) n' )
              ys
          |> List.fold_left (fun acc s -> acc ^ "" ^ s) "" )
      in
      let closure_creation = make_closure x' l (x' ^ "_env") in
      Printf.sprintf "%s%s%s" environment_creation closure_creation
        (trans_exp r rt ((x, t) :: t_env) typedef_names (x' ^ "_") func_name e)
    with Not_found ->
      print_endline ("ys: " ^ List.fold_left (fun acc s -> acc ^ ", " ^ s) "" ys) ;
      Printf.eprintf "t_env: %s"
        (List.fold_left (fun acc (x, y) -> acc ^ "," ^ x) "" t_env) ;
      assert false )
  | AppCls (x, ys) -> (
      let x' = alpha_convert x in
      if x' = func_name then
        ...
        try
          ...
          Printf.sprintf "  %s = ((%s*)%s -> fp)(%s, %s -> env, %s);" r typedef_type x'
            (list_params ys) x' x'
        with Not_found ->
          List.iter (fun s -> print_endline s) typedef_names ;
          print_endline types ;
          print_endline (typedef_of_type rt) ;
          print_endline (string_of_type rt) ;
          print_endline (get_return_type rt) ;
          print_endline (list_params ys) ;
          assert false )
  | AppDir (Id.L l, xs) -> (
      ...
      | "min_caml_create_float_array" | "min_caml_create_array" ->
          ...
          in
          Printf.sprintf "%s%s = make_%s_array(%s, %s);" cls r (type_for_array init_typ)
            (alpha_convert size) (alpha_convert init_n)
      ...
      | "min_caml_prerr_int" | "min_caml_prerr_float" | "min_caml_prerr_byte" ->
          Printf.sprintf "fprintf(stderr, %s);" params
      | _ -> Printf.sprintf "  %s = %s_fun(%s, NULL, NULL);" r (alpha_convert l) params )
  | Tuple xs ->
      Printf.sprintf "%s = safe_malloc(%d);\n%s" r (List.length xs)
        (set_tuple r t_env xs)
  | LetTuple (xts, y, e) ->
      let t = trans_exp r rt t_env typedef_names env_name func_name e in
      Printf.sprintf "%s%s" (create_tuple xts y) t
  | Get (x, y) ->
      Printf.sprintf "%s = %s[%s].%s;" r (alpha_convert x) (alpha_convert y)
        (type_for_union rt)
  | Put (x, y, z) ->
      let name, typ = List.find (fun (n, t) -> n = z) t_env in
      Printf.sprintf "%s[%s].%s = %s;" (alpha_convert x) (alpha_convert y)
        (type_for_union typ) (alpha_convert z)
...
let rec emit_funs (f : fundef list) (typedef_names : string list) =
  match f with
  | [] -> ""
  | _ :: _ ->
      let elem = List.hd f in
        ...
        Printf.sprintf "(%s, Value *env, Closure *%s)"
          (List.fold_left
             (fun acc (s, typ) ->
               ...
               | _ -> acc ^ ", " ^ string_of_type typ ^ " " ^ s' )
             "" a)
          l
      in
      let func_end = end_function "result" in
      let t_env = (l', t) :: List.append fv a in
      Printf.sprintf "%s%s{\n%s result;\n%s%s%s\n\n%s" name signature return_typ
        (set_environment fv)
        (trans_exp "result" rt t_env typedef_names "" l b)
        func_end
        (emit_funs (List.tl f) typedef_names)

(*This function creates the C main function*)
let emit_main r typedef_names body =
  Printf.sprintf "int main(){\nint %s = 0;\n%s\nreturn %s;\n}\n" r
    (trans_exp r Type.Int [] typedef_names "" "main" body)
    r

(*Compiles Min-Caml code through closure conversion, then translates the resulting
  intermediate code into C*)
let translate s =
  let funcs, mainf =
    Lexing.from_string s |> Parser.exp Lexer.token |> Typing.f |> KNormal.f |> Alpha.f
    |> Assoc.f |> Inline.f |> Elim.f |> Closure.f
    |> fun (Prog (p, t)) -> (p, t)
  in
  ...
  make_header ()
  ^ List.fold_right (fun acc s -> "typedef " ^ acc ^ ";\n" ^ s) typedefs ""
  ^ "\n" ^ !r ^ "\n" ^ emit_funs funcs typedef_names
  ^ emit_main "ans" typedef_names mainf
